...
class RDTFeeddownGUI(QMainWindow):
    ...
    def build_paths_section(self):
        ...
        self.change_input_path_button.setToolTip(
            "Select the default input path for analysis files, see Help for how to set it before launch"
        )
        paths_layout.addWidget(self.change_input_path_button)
        self.output_path_label = QLabel(
            f"Default Output Path: {self.default_output_path}"
        )
        ...
        self.change_output_path_button.setToolTip(
            "Select the default output path for analysis files, see Help for how to set it before launch"
        )
    ...
    def build_input_tab(self):
        ...
        self.beam1_model_button.clicked.connect(
            lambda: select_singleitem(
                self,
                "LHCB1",
                "Select LHCB1 Model",
        ...
        self.beam1_model_button.setToolTip(
            "Select LHCB1 model folder - needed only for valid BPM list"
        )
        ...
        self.beam2_model_button.clicked.connect(
            lambda: select_singleitem(
                self,
                "LHCB2",
                "Select LHCB2 Model",
        ...
        self.beam2_model_button.setToolTip(
            "Select LHCB2 model folder - needed only for valid BPM list"
        )
        ...
        self.beam1_reffolder_button.clicked.connect(
            lambda: select_singleitem(
                self,
                "LHCB1",
        ...
        self.beam1_reffolder_button.setToolTip(
            "Select LHCB1 OMC analysis folder to be used as the reference"
        )
        ...
        self.beam1_reffolder_remove_button.clicked.connect(
            lambda: self.remove_singlefolder(
                "LHCB1", self.beam1_reffolder_entry, self.beam2_reffolder_entry
            )
        )
        beam1_buttons_layout.addWidget(
            self.beam1_reffolder_remove_button
        )  # Added the remove button to the layout
        ...
        self.beam2_reffolder_button.clicked.connect(
            lambda: select_singleitem(
                self,
                "LHCB2",
        ...
        self.beam2_reffolder_button.setToolTip(
            "Select LHCB2 OMC analysis folder to be used as the reference"
        )
        ...
        self.beam2_reffolder_remove_button.clicked.connect(
            lambda: self.remove_singlefolder(
                "LHCB2", self.beam1_reffolder_entry, self.beam2_reffolder_entry
            )
        )
        ...
        self.beam1_folders_list.itemSelectionChanged.connect(
            lambda: self.update_select_all_checkbox(
                self.beam1_folders_list, self.beam1_select_all_checkbox
            )
        )
        beam1_buttons_layout = QHBoxLayout()
        self.beam1_folders_button = QPushButton("Add Folders")
        self.beam1_folders_button.clicked.connect(self.select_beam1_folders)
        self.beam1_folders_button.setToolTip(
            "Select LHCB1 OMC analysis folders to be used as comparison with reference - needs more than 3 different xing angles"
        )
        ...
        self.beam1_select_all_checkbox.stateChanged.connect(
            self.toggle_select_all_beam1_folders
        )
        ...
        self.beam2_folders_list.itemSelectionChanged.connect(
            lambda: self.update_select_all_checkbox(
                self.beam2_folders_list, self.beam2_select_all_checkbox
            )
        )
        beam2_buttons_layout = QHBoxLayout()
        self.beam2_folders_button = QPushButton("Add Folders")
        self.beam2_folders_button.clicked.connect(self.select_beam2_folders)
        self.beam2_folders_button.setToolTip(
            "Select LHCB2 OMC analysis folders to be used as comparison with reference - needs more than 3 different xing angles"
        )
        ...
        self.beam2_select_all_checkbox.stateChanged.connect(
            self.toggle_select_all_beam2_folders
        )
        ...
        self.validate_knob_button.setToolTip(
    ...
    def build_validation_tab(self):
        ...
        self.validation_files_list.itemSelectionChanged.connect(
            lambda: self.update_select_all_checkbox(
                self.validation_files_list, self.validation_select_all_checkbox
            )
        )
        ...
        self.validation_files_remove_button.clicked.connect(
            lambda: self.remove_selected_items(self.validation_files_list)
        )
        ...
        self.validation_select_all_checkbox.stateChanged.connect(
            lambda state: self._toggle_select_all(self.validation_files_list, state)
        )
        ...
        load_selected_files_button.setToolTip(
            "Load selected analysis files for plotting (if files are not loaded, plotting of them is not possible)"
        )
        self.validation_layout.addWidget(load_selected_files_button)

        self.loaded_files_list = QTreeWidget()
        self.loaded_files_list.setHeaderLabels(
            ["Filename", "Beam", "RDT", "RDT plane", "Xing Knob Name"]
        )
        ...
        self.search_bpm_button.setToolTip(
            "Only required if want to check if BPM exists in data"
        )
        ...
        self.plot_rdt_button.clicked.connect(self.plot_gui_rdt)
    ...
    def build_correction_tab(self):
        ...
        self.correction_loaded_files_list.setHeaderLabels(
            ["Filename", "Beam", "RDT", "RDT plane", "Corrector"]
        )
        self.correction_loaded_files_list.setSelectionMode(
            QAbstractItemView.ExtendedSelection
        )
        loaded_files_layout.addWidget(self.correction_loaded_files_list)
        self.correction_loaded_files_list.itemSelectionChanged.connect(
            lambda: self.update_select_all_checkbox(
                self.correction_loaded_files_list, self.select_all_files_checkbox
            )
        )
        ...
        self.load_file_button.setToolTip(
            "Load selected response files to use to match to the analysis files"
        )
        ...
        self.remove_file_button.clicked.connect(
            lambda: self.remove_selected_items(
                self.correction_loaded_files_list,
                self.corr_responses,
                corr_responses=True,
            )
        )
        ...
        self.select_all_files_checkbox.stateChanged.connect(
            lambda state: self._toggle_select_all(
                self.correction_loaded_files_list, state
            )
        )
        ...
        b1_button.setToolTip(
            "Select LHCB1 analysis file outputted in the Validation tab from the measurements to be matched to with the response files"
        )
        ...
        b2_button.setToolTip(
            "Select LHCB2 analysis file outputted in the Validation tab from the measurements to be matched to with the response files"
        )
        ...
        self.corr_beam1_reffolder_button.clicked.connect(
            lambda: select_singleitem(
                self,
                "LHCB1",
        ...
        self.corr_beam1_reffolder_button.setToolTip(
            "Select LHCB1 OMC analysis folder to be used as the reference"
        )
        ...
        self.corr_beam1_reffolder_remove_button.clicked.connect(
            lambda: self.remove_singlefolder(
                "LHCB1",
                self.corr_beam1_reffolder_entry,
        ...
        self.corr_beam2_reffolder_button.clicked.connect(
            lambda: select_singleitem(
                self,
                "LHCB2",
        ...
        self.corr_beam2_reffolder_button.setToolTip(
            "Select LHCB2 OMC analysis folder to be used as the reference"
        )
        ...
        self.corr_beam2_reffolder_remove_button.clicked.connect(
            lambda: self.remove_singlefolder(
                "LHCB2",
                self.corr_beam2_reffolder_entry,
                self.corr_beam2_reffolder_entry,
        ...
        self.corr_beam1_measfolder_button.clicked.connect(
            lambda: select_singleitem(
                self,
                "LHCB1",
        ...
        self.corr_beam1_measfolder_button.setToolTip(
            "Select LHCB1 OMC analysis folder which only has a different XING angle to the reference"
        )
        ...
        self.corr_beam1_measfolder_remove_button.clicked.connect(
            lambda: self.remove_singlefolder(
                "LHCB1",
                self.corr_beam1_measfolder_entry,
        ...
        self.corr_beam2_measfolder_button.clicked.connect(
            lambda: select_singleitem(
                self,
                "LHCB2",
        ...
        self.corr_beam2_measfolder_button.setToolTip(
            "Select LHCB2 OMC analysis folder which only has a different XING angle to the reference"
        )
        ...
        self.corr_beam2_measfolder_remove_button.clicked.connect(
            lambda: self.remove_singlefolder(
                "LHCB2",
                self.corr_beam2_measfolder_entry,
                self.corr_beam2_measfolder_entry,
        ...
        self.b1_corr_knob_entry.setToolTip(
            "Enter the corrector value for LHCB1 (should be same for both reference and response)"
        )
        ...
        self.b1_corr_xing_entry.setToolTip(
            "Enter the difference in xing angle for LHCB1 (between reference and response)"
        )
        ...
        self.b2_corr_knob_entry.setToolTip(
            "Enter the corrector value for LHCB2 (should be same for both reference and response)"
        )
        ...
        self.b2_corr_xing_entry.setToolTip(
            "Enter the difference in xing angle for LHCB2 (between reference and response)"
        )
        ...
        self.corr_knob_entry.setToolTip(
            "Enter the corrector value (should be same for both reference and response)"
        )
    ...
    def change_default_input_path(self):
        new_path = QFileDialog.getExistingDirectory(
            self, "Select Default Input Path", self.default_input_path
        )
        if new_path:
            self.default_input_path = new_path
            self.input_path_label.setText(
                f"Default Input Path: {self.default_input_path}"
            )

    def change_default_output_path(self):
        new_path = QFileDialog.getExistingDirectory(
            self, "Select Default Output Path", self.default_output_path
        )
        if new_path:
    ...
    def validate_knob_button_clicked(self):
        ...
        if is_valid_knob:
            QMessageBox.information(
                self, "Knob Validation", "Knob is valid. Setting: " + repr(knob_message)
            )
        else:
            self.log_error(
                "Knob Validation", "Invalid Knob: " + repr(knob_message), knob_message
            )
        self.input_progress.hide()
            ...
            if reply == QMessageBox.Yes:
                ...
                for file in selected_files:
                    ...
                    valid = validate_file_structure(
                        data,
                        ["beam", "ref", "rdt", "rdt_plane", "knob"],
                        self.log_error,
                    )
                    if not valid:
                        matches = self.validation_files_list.findItems(
                            file, Qt.MatchExactly
                        )
                        if matches:
                            for item in matches:
    ...
    def search_bpm(self):
        ...
        if data is None:
            QMessageBox.information(
                self, "BPM Search", f"No data available for {beam}."
            )
            return
        if search_term in data["data"]:
            QMessageBox.information(
                self, "BPM Search", f"Found BPM '{search_term}' in {beam}."
            )
        else:
            QMessageBox.information(
                self, "BPM Search", f"BPM '{search_term}' not found in {beam}."
            )
    ...
    def graph_bpm(self):
        ...
        beam = self.beam_selector.currentText()
        data = (
            getattr(self, "b1rdtdata", None)
            if beam == "LHCB1"
            else getattr(self, "b2rdtdata", None)
        )
        if data is None:
    ...
    def plot_gui_rdt(self):
        ...
        self.rdt_axes, self.rdt_axes_layout = self.setup_figure(
            self.rdtWidget, datab1, datab2, 3
        )
    ...
    def plot_rdt_shifts(self):
        ...
        self.rdtshift_axes, self.rdtshift_axes_layout = self.setup_figure(
            self.rdtShiftwidget, datab1, datab2, 3
        )
    ...
    def select_properties_file(self):
        filename, _ = QFileDialog.getOpenFileName(
            ...
            self.default_input_path,
            "Properties Files (*.csv);;All Files (*)",
        )
        if filename:
    ...
    def remove_selected_items(self, widget, saved_data=None, corr_responses=False):
        ...
        if isinstance(widget, QListWidget):
            ...
            raise TypeError(
                "Unsupported widget type. Only QListWidget and QTreeWidget are supported."
            )
    ...
    def _toggle_select_all(self, widget, state):
        ...
        if isinstance(widget, QTreeWidget):
            ...
            raise TypeError(
                "Unsupported widget type. Only QTreeWidget and QListWidget are supported."
            )
    ...
    def select_beam1_folders(self):
        select_folders(
            self,
            self.default_input_path,
            "Beam1BunchTurn (Beam1BunchTurn*);;All Folders (*)",
            self.beam1_folders_list,
        )
    ...
    def select_beam2_folders(self):
        select_folders(
            self,
            self.default_input_path,
            "Beam2BunchTurn (Beam2BunchTurn)*;;All Folders (*)",
            self.beam2_folders_list,
        )
    ...
    def plot_loaded_correction_files(self):
        ...
        if self.b1_response_meas:
            if (
                self.b1_response_meas.get("metadata", {}).get("rdt") == self.rdt
                and self.b1_response_meas.get("metadata", {}).get("rdt_plane")
                == self.rdt_plane
                and self.b1_response_meas.get("metadata", {}).get("beam")[-1] == "1"
            ):
                self.b1_response_meas["data"] = dict(
                    self.b1_response_meas["data"].items()
                )
            else:
                self.b1_response_meas[
                    "data"
                ] = {}  # Clear data if metadata doesn't match
                self.log_error("LHCB1 response measurement metadata does not match.")
                self.simcorr_progress.hide()
                return
        if self.b2_response_meas:
            if (
                self.b2_response_meas.get("metadata", {}).get("rdt") == self.rdt
                and self.b2_response_meas.get("metadata", {}).get("rdt_plane")
                == self.rdt_plane
                and self.b2_response_meas.get("metadata", {}).get("beam")[-1] == "2"
            ):
                self.b2_response_meas["data"] = dict(
                    self.b2_response_meas["data"].items()
                )
            else:
        ...
        if self.b1andb2same_checkbox.isChecked() and (
            not self.b1_response_meas or not self.b2_response_meas
        ):
        ...
        if self.b1andb2same_checkbox.isChecked() and (
            self.b1data is None or self.b2data is None
        ):
            self.log_error("Both beams must be loaded for reference measurement.")
            self.simcorr_progress.hide()
            return

        self.corr_axes, grid = self.setup_figure(
            self.figureContainer, self.b1data, self.b2data, 2
        )
        QApplication.processEvents()
    ...
    def load_selected_correction_files(self):
        ...
        selected_files = select_multiple_treefiles(
            self,
            self.correction_loaded_files_list,
            title="Select Response Files",
            saved_data=self.corr_responses,
        )
    ...
    def update_knobs_and_replot(self):
        ...
        if not hasattr(self, "corr_axes"):
            self.log_error(
                "Correction axes not defined. Please plot the correction files first."
            )
            self.simcorr_progress.hide()
            return
        ...
        knob_values = {
            knob: widget.text() for knob, widget in self.knob_widgets.items()
        }
        plot_drdt_dknob(
            self.b1data,
            self.b2data,
