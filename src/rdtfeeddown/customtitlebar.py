...
def create_custom_title_bar(parent: type):
    ...
    error_log_button.setIcon(
        parent.style().standardIcon(QStyle.SP_FileDialogDetailedView)
    )
    ...
    if event.button() == Qt.LeftButton:
        ...
        if isNearEdge(parent, local_pos):
            ...
            parent._resize_start_pos = (
                event.globalPos()
            )  # keep global for delta calculations
            parent._resize_start_geom = parent.geometry()
            parent._resize_direction = get_resize_direction(parent, local_pos)
        else:
...
def mouseMoveEvent(parent: type, event: QMouseEvent):  # noqa: N802
    if parent._resizing:
        ...
        local_pos = parent.mapFromGlobal(event.globalPos())
        direction = get_resize_direction(parent, local_pos)
...
def handle_resize(parent: type, global_pos: QPoint):
    ...
    direction = parent._resize_direction
