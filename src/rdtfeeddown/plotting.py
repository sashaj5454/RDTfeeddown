import numpy as np
from pyqtgraph import ErrorBarItem, PlotDataItem, TextItem, mkBrush, mkPen
...
def plot_bpm(bpm, fulldata, rdt, rdt_plane, ax1=None, ax2=None, log_func=None):
    try:
        ...
        error_ax1 = ErrorBarItem(
            x=xing,
            y=re,
            height=2 * amp_err,
            beam=0.1,
            pen=mkPen(line_color, width=2),
            capSize=15,
        )
...
def plot_avg_rdt_shift(ax, data, line_color, rdt, rdt_plane, knob):
    ...
    set_axis_label(
        ax,
        "left",
        f"√(ΔRe(f<sub>{rdt_plane},{rdt}</sub>)<sup>2</sup> + ΔIm(f<sub>{rdt_plane},{rdt}</sub>)<sup>2</sup>)",
    )
    set_axis_label(ax, "bottom", f"{knob} trim")
    ax.plot(
        xing, ampdat, pen=line_color, symbol="x", symbolPen=line_color
    )  # Plot the data points.
    error_item = ErrorBarItem(
        x=xing, y=ampdat, top=stddat, bottom=stddat, beam=0.1, pen=line_color
    )
...
def plot_rdtshifts(b1data, b2data, rdt, rdt_plane, axes, knob, log_func=None):
    ...
    try:
        ...
        def plot_beam_data(axs, data, label, knob=knob):
            ...
            if not ax_re.getAxis(
                "left"
            ).labelText:  # Check if the Y-axis label is already set
                set_axis_label(
                    ax_re, "left", f"∂Re(f<sub>{rdt_plane},{rdt}</sub>)/∂knob"
                )
            if not ax_re.getAxis(
                "bottom"
            ).labelText:  # Check if the X-axis label is already set
                ax_re.setLabel("bottom", "S", units="km")
            # Set labels for the imaginary part plot
            if not ax_im.getAxis(
                "left"
            ).labelText:  # Check if the Y-axis label is already set
                set_axis_label(
                    ax_im, "left", f"∂Im(f<sub>{rdt_plane},{rdt}</sub>)/∂knob"
                )
            if not ax_im.getAxis(
                "bottom"
            ).labelText:  # Check if the X-axis label is already set
    ...
    try:
        ...
        def plot_single_beam(ax_amp, ax_re, ax_im, data, beam_label):
            ...
            for i, angle in enumerate(xing):
                ...
                hover_scatter_amp = HoverLine(
                    x=sdat,
                    y=ampdat,
                    label=f"XING angle = {angle}",
                    pen=colour,
                    symbol="x",
                    symbolPen=colour,
                )
        ...
        if b1data and b2data:
            # Plot B1 (left column)
            hover_lines_amp_b1, hover_lines_re_b1, hover_lines_im_b1 = plot_single_beam(
                ax1, ax3, ax5, b1data, "LHCB1"
            )
            ...
            hover_lines_amp_b2, hover_lines_re_b2, hover_lines_im_b2 = plot_single_beam(
                ax2, ax4, ax6, b2data, "LHCB2"
            )
...
def plot_drdt_dknob(b1data, b2data, rdt, rdt_plane, axes, knoblist=None, log_func=None):
    ...
    try:
        ...
        def plot_beam_data(axs, data, label):
            ...
            is_file_key_structure = isinstance(
                next(iter(data.values())), dict
            ) and "data" in next(iter(data.values()))
            sdat, dredkdat, dimdkdat = [], [], []
            dredkerr, dimdkerr = [], []
            ...
            if line_label == "Simulation":
                hover_scattera = HoverLine(
                    x=sdat, y=dredkdat, label=line_label, pen="g"
                )
                ax_re.addItem(hover_scattera)
                hover_line_scatter_re.append(hover_scattera)
                hover_scatterb = HoverLine(
                    x=sdat, y=dimdkdat, label=line_label, pen="g"
                )
        ...
        # Plot for both beams
        if b1data and b2data:
            hover_line_scatter_re_b1, hover_line_scatter_im_b1 = plot_beam_data(
                (ax1, ax3), b1data, "LHCB1"
            )
            install_closest_y_hover(ax1, hover_line_scatter_re_b1)
            install_closest_y_hover(ax3, hover_line_scatter_im_b1)

            hover_line_scatter_re_b2, hover_line_scatter_im_b2 = plot_beam_data(
                (ax2, ax4), b2data, "LHCB2"
            )
        ...
        if log_func:
            log_func(
                f"Error plotting dRDTdknob for f<sub>{rdt_plane},{rdt}</sub>: {e}", e
            )
        else:
...
def install_closest_y_hover(ax, hover_lines):
    ...
    def mouse_moved(evt):
        scene_pos = evt.scenePos() if hasattr(evt, "scenePos") else evt
        ...
        y_mins = [
            ...
            if (ydata := line.getData()[1]) is not None and len(ydata) > 0
        ]
        y_maxs = [
            np.max(ydata)
            for line in hover_lines
    ...
    vb.scene().sigMouseMoved.connect(mouse_moved)
